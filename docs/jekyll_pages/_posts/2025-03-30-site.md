...
(7) d'avoir accès en écriture au <https://github.com/tdjeunes/website>. Notez que vous aurez accès en écriture à toutes les branches sauf `master`. Si vous voulez faire un changement à master, vous devez ouvrir une nouvelle branche, et ouvrir une PR.
...
## Étape 1: installez une version locale du site sur la branche master
...
```
git clone git@github.com:tdjeunes/website.git
cd tdj-comm
```

(À noter qu'il est possible que dans votre cas vous utilisiez "https://github.com/tdjeunes/website.git" plutôt que "git@github.com:tdjeunes/website.git". Si ça marche, c'est OK.)
...
```
[remote "origin"]
  url = git@github.com:tdjeunes/website.git
```
...
```
[remote "origin"]
  url = https://github.com/tdjeunes/website.git
```
...
```
Primary remote repository (origin): git@github.com:tdjeunes/website.git
```

ou encore:

```
Primary remote repository (origin): https://github.com/tdjeunes/website.git
```

(L'important est que voyiez "tdjeunes/website". Si vous voyez autre chose, comme par exemple, "CECI-EST-MON-NOM-DUTILISATEUR-GITHUB/tdj-comm", c'est que vous utiliesz un "fork" du répertoire principal. Cette façon de faire n'est pas recommandée ou supportée par cette documentation; si vous utilisez cette technique, assurez-vous de bien comprendre les concepts de synchronisation de branches entre forks GitHub.)

## Étape 3: assurez-vous d'être sur la branche master
...
Sinon revenez sur la branche master en faisant:
...
Allez au https://github.com/tdjeunes/website et vous verrez quelque chose comme:
...
```
commit 4b0378fa826df26ae08d2ca3a143c7c06de56fb3 (HEAD -> master, github/master)
```
...
remote:      https://github.com/tdjeunes/website/pull/new/71
remote:
To github.com:tdjeunes/website.git
* [new branch]      71 -> 71
```
...
```
https://github.com/tdjeunes/website/pull/new/71
```
...
* Allez à https://github.com/tdjeunes/website/pulls
* Trouvez votre PR
* Cliquez dessus
...
* changer la date du copyright pour que ce soit toujours l'année en cours -- ceci est mauvais car ça en fait pas référence à une tâche
  ...
  14.6. dans l'onglet principal, conversation, il ne doit pas y avoir des conflits avec la branche master.
